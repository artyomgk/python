...
with open("text_77.json", "w", encoding="utf-8") as f_obj:
    with open("text_7.txt", encoding="utf-8") as f_obj1:
        profit = {line.split()[0]: int(line.split()[2]) - int(line.split()[3]) for line in f_obj1}
        result = [profit, {"average_profit": round(sum([int(i) for i in profit.values() if int(i) > 0]) /
                                               len([int(i) for i in profit.values() if int(i) > 0]))}]
    json.dump(result, f_obj, ensure_ascii=False, indent=4)
